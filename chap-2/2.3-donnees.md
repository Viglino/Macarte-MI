...
<i class="fg-polygon-pt fg-3x fleft"></i> La surface (ou polygone) apparaît sur la carte comme une unité distinctive d'un ensemble (fleuve, mer, lac), comme une zone étendue ayant un ensemble de propriétés homogènes (même occupation du sol, même règlementation urbaine, etc.).
...
<i class="fa fa-text-width fa-3x fleft"></i> En cartographie, on peut ajouter l'annotation à cette liste. Généralement associée à une des géométrie précédement citée, elle se caratérise par le fait qu'elle est représentée sur la carte par un texte et non par la géométrie qu'elle supporte.
...
Elle peut avoir une représentation distincte de l'objet qu'elle désigne et avoir une géométrie différente (ligne dans une surface), pour des raisons de lisibilité ou pour permettre de la manipuler différemment de l'objet qu'elle représente (afficher/masquer, faire appraitre à partir d'un certain zoom, etc.).
...
Les couches de référence ou de dessin sont disponible dans le gestionnaire de couche (pour en modifer l'ordre, la visibilité ou la transparence par exemple). Elles sont parfois appélées "*calques*" par analogie aux papiers calques qu'on superpose dans un processus de reproduction pour former une image.
...
C'est d'autant plus légitime qu'avec l'ouverture des données ([mouvement Opendata](https://www.cnil.fr/fr/publication-en-ligne-et-reutilisation-des-donnees-publiques-open-data)) le nombre d'organisme et de plateforme proposant de télécharger des données s'est multipllié sur le web.   
En dehors des flux cartographiques mis à disposition que nous avons [vu précédemment](/Macarte-MI/chap-2/2.1-fonds.html), on peut aissi télécharger directement des données à caratère géographique via des fichiers diffusés sur le net. On distinguera les formats de données géographique, exploitable directement, qui contiennent la représentation géométrique des objets et les données attributaires qui demanderontun traitement préalable pour être affiché sur une carte.
...
A l'inverse, vous pouvez exporter les données saisies dans Ma carte dans le dialogue de configuration la couche <i class="fa fa-cog outline"></i> dans le gestionnaire de calques) et en cliquant sur l'icone d'export <i class="fa fa-download"></i>.
...
Le fichier GPX enregistre la liste de points de cheminements (latitude, longitude, altitude et éventuellement un horodatage) et conviendra pour échanger des points de rendez-vous ou des cheminents.
...
Lors d'un glissé-dépossé dans l'éditeur de carte, vous pouvez recharger toute la carte ou seulement une couche ou la légende ou la bibliothèque de symbole.
...
{: .warning }
Attention cependant, cette opération n'est pas magique et l'adresse contenue dans le fichier d'origine doit être suffisement précise et bien rédigée pour ne pas provoquer une ambiguïté dans la recherche.
...
{: .note }
Le site [data.gouv.fr](https://www.data.gouv.fr/fr/pages/donnees-geographiques/) recence et référence un grand nombre de données à composante géographique.
