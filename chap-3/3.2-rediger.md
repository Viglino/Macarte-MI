...
Comme nous l'avons vu précédemment, la première étape est de [bien choisir ses fonds](../chap-2/2.1-fonds.html#choisir-ses-fonds) qui vont le plus souvent servir à asseoir ses données. Dans Ma carte, vous avez accès à tous les fonds IGN et vous pouvez ajouter [les votre](../chap-2/2.1-fonds.html#ajouter-dautres-fonds).

Une fois ceux-ci définis, vous pouvez ajouter vos propre données.

## Ajouter des données

Dans les cas simples vous pouvez saisir les données à la main, mais la plupart du temps il sera plus facile de récupérer des données existantes dans une base de donnée ou parmi celles proposées en [Opendata](../chap-2/2.3-donnees.html#opendata-et-c%C3%A6tera).
...
Il vous faudra parfois les enrichir ou au contraire filter l'information qui vous intéresse, croiser les données entre elles, les organiser, les hiérarchiser.   

{: .note }
Si le contenu principale est la géo-données pensez aussi aux autres types de données qui viendront agrémenter le discourt (vidéos, photos ou texte) et la façon de les lier à la donnée.

Les données devront être réparties en couche en fonction des traitements que vous voulez permettre ou simplement pour faciliter leur visualisation.   
...
Pour des raisons de lisibilité, il faudra éventuellement définir à quel niveau de zoom la donnée va s'afficher ou s'il faut faire des regroupements (cluster) des données trop proches.
...
Plusieurs solutions existent pour mettre en avant de la donnée ou, au contraire, estomper les données secondaires tout sans perdre le contexte. Les variables visuelles qui assure [la différentiation](../chap-2/2.2-variables.html#bien-utiliser-les-variables-visuelles) en font partie.
...
La légende n'est pas forcément nécessaire sur toutes les cartes, mais elle facilite la représentation de l'information sur une carte et permet d'expliquer le thème.  
Dans Ma carte, elle se fait à partir des symbolisations stockées dans la bibliothèque, elle peut être chargée depuis une autre carte.
